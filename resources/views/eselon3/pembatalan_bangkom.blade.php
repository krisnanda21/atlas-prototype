            ...
            <tr class="cancel-row" data-title="{{ strtolower($p->nama_kegiatan) }}" data-competency="{{ $p->kompetensi_dasar }}">
                ...
                <td style="display:flex;gap:6px;align-items:center;">
                    @if($statusLower === 'pembatalan diajukan')
                        @if($canVerifLevel1)
                            <button type="button" class="btn btn-sm btn-danger" onclick="showConfirmDelete('{{ route('eselon3.approvePembatalan', $p->id) }}', '{{ addslashes($p->nama_kegiatan) }}')">✅ Approve </button>
                            <button type="button" class="btn btn-sm btn-neutral" onclick="showConfirmRejectPembatalan('{{ route('eselon3.rejectPembatalan', $p->id) }}', '{{ addslashes($p->nama_kegiatan) }}')">Reject</button>
                        @else
