...
<div class="card mb-4" style="padding:16px;">
    <form method="GET" style="display:flex;gap:10px;align-items:flex-end;">
        <div class="form-group" style="margin-bottom:0;flex:1;">
            <label class="form-label">Cari Pegawai (Nama, NIP, Jabatan, Unit Kerja)</label>
        ...
        @if($search) <a href="{{ route('eselon3.reviewIdp') }}" class="btn btn-neutral" style="height:40px;line-height:24px;">Reset</a> @endif
    </form>
</div>

<div class="card">
    <div class="card-title">Daftar Pegawai & IDP</div>
...
<form id="form-batch-agree" method="POST" action="{{ route('eselon3.agreeBatchIdp') }}" style="display:none;">
    @csrf
</form>
...
function openRevisiModal(id, need) {
    ...
    document.getElementById('revisi-form').action = `/eselon3/review-idp/${id}/revise`;
...
function openSingleAgreeModal(id, employeeName, need) {
    ...
    if (btnSubmit) {
        btnSubmit.onclick = function() {
            ...
            // For Kepala Unit routes
            form.action = `/eselon3/review-idp/${id}/agree`;
            form.submit();
        };
    }
